//Note: This is a slightly modified version of the original mousemanager.cpp.
//The only change is the class name and to make it independent of
//            wxSYS_DRAG_* constants on MAC OS X where those are not set.
//We are not sure how this modification affects the licensing but it's a
//             small modification in good spirit and we're not profiting.


///////////////////////////////////////////////////////////////////////////////
// Name:        src/common/mousemanager.cpp
// Purpose:     implementation of wxMouseEventsManagerMod class
...
#include "mousemanagermod.h"

// ----------------------------------------------------------------------------
// event tables
// ----------------------------------------------------------------------------

BEGIN_EVENT_TABLE(wxMouseEventsManagerMod, wxEvtHandler)
    EVT_MOUSE_CAPTURE_LOST(wxMouseEventsManagerMod::OnCaptureLost)
    EVT_LEFT_DOWN(wxMouseEventsManagerMod::OnLeftDown)
    EVT_LEFT_UP(wxMouseEventsManagerMod::OnLeftUp)
    EVT_MOTION(wxMouseEventsManagerMod::OnMove)
END_EVENT_TABLE()

// ============================================================================
// wxMouseEventsManagerMod implementation
// ============================================================================

void wxMouseEventsManagerMod::Init()
{
    m_win = NULL;
...
bool wxMouseEventsManagerMod::Create(wxWindow *win)
{
...
wxMouseEventsManagerMod::~wxMouseEventsManagerMod()
{
    if ( m_win )
        m_win->RemoveEventHandler(this);
}

void wxMouseEventsManagerMod::OnCaptureLost(wxMouseCaptureLostEvent& WXUNUSED(event))
{
    switch ( m_state )
    {
        case State_Normal:
...
void wxMouseEventsManagerMod::OnLeftDown(wxMouseEvent& event)
{
...
void wxMouseEventsManagerMod::OnLeftUp(wxMouseEvent& event)
{
    switch ( m_state )
    {
        case State_Normal:
...
void wxMouseEventsManagerMod::OnMove(wxMouseEvent& event)
{
    switch ( m_state )
    {
        case State_Normal:
...
#ifdef __APPLE__
                static const int dragMinX = 6;
                static const int dragMinY = 6;
#else
                static const int
